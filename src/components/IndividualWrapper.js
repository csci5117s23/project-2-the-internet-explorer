...
export default function IndividualWrapper({ router }) {
  ...
  //     console.log('trip id in getIndividualTrip: ', tripID);
  //     if (tripID) {
  //       setLoadingCurTrip(true);
  //       try {
  //         if (userId) {
  //           const token = await getToken({ template: "codehooks" });
  ...
  if (tripData) {
    ...
    // } else {
      ...
      if (tripData.length == 1) {
        console.log('trip id in data length 1: ', tripID);
        console.log('cur trip: ', curTrip);
    ...
    return <HomePage loadingTrips={loadingTrips} allTrips={allTrips} setAllTrips={setAllTrips}></HomePage>
  }
  // }
