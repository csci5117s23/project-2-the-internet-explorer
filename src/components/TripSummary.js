...
export default function TripSummary({ trip, tripMemories, closeModal }) { 
  ...
  const [firstMemoryCoordinates, setFirstMemoryCoordinates] = useState(null);

  const handleLeftClick = () => {
    const keys = ["All locations", ...Object.keys(coordinatesDict)];
  ...
  useEffect(() => {
    ...
    let minDate = null;
    let maxDate = null;
    let firstMemoryCoordinates = null;
    if (tripMemories) {
      for (let memory of tripMemories) {
        ...
        if (!firstMemoryCoordinates && memory.latitude && memory.longitude) {
          firstMemoryCoordinates = { lat: memory.latitude, lng: memory.longitude };
          setFirstMemoryCoordinates(firstMemoryCoordinates);
        }
    ...
    if (Object.keys(coordinatesDict).length > 0) {
      setCurrentLocationKey("All locations");
    }
  }, [tripMemories]);

  useEffect(() => {
    if (currentLocationKey === "All locations") {
      setZoom(4);
    } else {
      setZoom(10);
    }
      ...
      <div className={`p-2 ${styles.tripSummaryContainer}`}>
        <TripSummaryMap 
          ...
          currentCoordinate={ 
            ...
            : coordinatesDict[currentLocationKey]
          }
          zoom={zoom}
        />
        ...
        <div className="p-4">
          ...
          <div className="bg-gray-200 rounded-lg shadow-sm p-2 mt-2 mb-4">
            <div className="text-lg font-bold mb-2 bg-gray-300 p-2 m-1 rounded-md">
              ...
              <h1 className={`text-l font-bold ${styles.tripSummaryData}`}> <FontAwesomeIcon icon={faCalendarDay} /> # of Events: {memoriesCategoryCount && memoriesCategoryCount.events ? memoriesCategoryCount.events : "You have no events"} </h1>
